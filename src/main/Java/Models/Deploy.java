        ...
        if (valid(p_gameState)) {
            for (Country l_country : p_gameState.getD_map().getD_countries()) {
                if (l_country.getD_countryName().equalsIgnoreCase(this.d_targetCountryName)) {
                    ...
                    this.setD_orderExecutionLog(STR."\{+l_armiesToUpdate} armies have been successfully deployed on the Country : \{l_country.getD_countryName()}",
                            "default");
                }
            }

        } else {
            this.setD_orderExecutionLog(STR."Deploy Order = deploy \{this.d_targetCountryName} \{this.d_numberOfArmiesToPlace} is not executed since Target country: \{this.d_targetCountryName} given in deploy command does not belongs to the player : \{d_playerInitiator.getPlayerName()}", "error");
